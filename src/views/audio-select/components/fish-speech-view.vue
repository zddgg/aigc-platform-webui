...
const handleQueryModels = async () => {
  ...
  modelDataOptions.value = fsRes.data.reduce((acc: any, item) => {
...
const computedConfigs = computed(() => {
  return [{label: '空', value: '-1'}, ...fsConfigs.value];
})
...
watch(
    () => props.activeTabKey,
    () => {
      if (props.activeTabKey !== FISH_SPEECH) {
        stopAudio();
      }
    },
    {immediate: true}
