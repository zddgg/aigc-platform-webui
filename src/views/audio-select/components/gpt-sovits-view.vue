...
const handleQueryModels = async () => {
  ...
  modelDataOptions.value = gsvRes.data.reduce((acc: any, item) => {
...
const computedConfigs = computed(() => {
  return [{label: '空', value: '-1'}, ...gsvConfigs.value];
})
...
const modelSelect = (mood: Mood) => {
  ...
  const audioModelConfig: AudioModelConfig = {
    audioModelType: GPT_SOVITS,
    audioModelId: currentModel.value[1],
...
watch(
    ...
    async () => {
      if (props.visible) {
        ...
        if (props.activeTabKey === GPT_SOVITS && props.audioModelConfig) {
          modelDataOptions.value.forEach(item => {
            item.children?.forEach((item1) => {
              if (item1.value === props.audioModelConfig?.audioModelId) {
...
watch(
    () => props.activeTabKey,
    () => {
      if (props.activeTabKey !== GPT_SOVITS) {
        stopAudio();
      }
    },
    {immediate: true}
